...
%            end
  %%
        Err_Rand1(kk,i+L)=(w_Rand1-w1)'*(w_Rand1-w1);
        ek_Rand1=dk-w_Rand1'*uk;
%         if i<Begin
%             w_Rand1=w_Rand1+mu_LMS*ek_Rand1*uk;
%         else
...
%            end
   %% 对比的算法
          %% LMF
        Err_LMF(kk,i+L)=(w_LMF-w1)'*(w_LMF-w1);
        ek_LMF = dk- w_LMF'*uk;
%           if i<Begin
%             w_LMF=w_LMF+mu_LMS*ek_LMF*uk;
%         else
...
%         end
  %% MEE
        ...
        TH_Rand(kk,i+L)=mu_Rand^2*5*R_two/(1-(1-mu_Rand*R_two)^2);
                 TH_Rand1(kk,i+L)=mu_Rand1^2*5*R_four/(1-(1-mu_Rand1*R_four)^2);
    end
    disp(kk);
end
toc

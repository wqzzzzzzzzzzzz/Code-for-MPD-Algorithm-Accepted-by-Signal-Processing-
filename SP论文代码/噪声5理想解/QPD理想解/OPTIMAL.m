...
for kk=1:10 %做多次实验取平均值
    ...
    for i=1:L
        ...
        if i<Begin
            ...
            w_Rand=w_Rand+mu_Rand*R1*uk;
                ...
                w_Rand=w_Rand+mu_Rand*R1*uk/i*2000;
            end
        end
    ...
    i4 = 0;
    for ii = 5001 : L
        if V_Rand(ii,1) ==max(V_Rand(ii,:))
            i1 = i1 + 1;
            U1(:, i1) = UU(:, ii);
            D1(i1) = w0' * UU(:, ii) + VV(ii)-MIU_EM(1);
        elseif V_Rand(ii,2) ==max(V_Rand(ii,:))
            i2 = i2 + 1;
            U2(:, i2) = UU(:, ii);
            D2(i2) = w0' * UU(:, ii) + VV(ii)-MIU_EM(2);
        elseif V_Rand(ii,3) ==max(V_Rand(ii,:))
            i3 = i3 + 1;
            U3(:, i3) = UU(:, ii);
            D3(i3) = w0' * UU(:, ii) + VV(ii)-MIU_EM(3);
        else
            i4 = i4 + 1;
            U4(:, i4) = UU(:, ii);
            D4(i4) = w0' * UU(:, ii) + VV(ii)-MIU_EM(4);
        end
    end

    a1=WEIGHT_EM(1)/SIGMA_EM(1)^2;
    a2=WEIGHT_EM(2)/SIGMA_EM(2)^2;
    a3=WEIGHT_EM(3)/SIGMA_EM(3)^2;
    a4=WEIGHT_EM(4)/SIGMA_EM(4)^2;

    w_em_th = inv(a1 * U1 * U1' /i1 + a2 * U2 * U2' /i2+ a3 * U3 * U3' /i3+ a4 * U4 * U4' /i4) * (a1 * U1 * D1' /i1 + a2 * U2 * D2' /i2+ a3 * U3 * D3' /i3+ a4 * U4 * D4' /i4);
...
legend('QPD', 'Wiener Solution','Optimal Solution (44)');
xlabel('Iteration');
ylabel('MSD(dB)');
box on;